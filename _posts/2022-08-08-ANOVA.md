...
- 只比较两个独立均值差异时， F 检验和 t 检验结果相同，自由度也相同，其中 $F=t^2$ 。 
...
- 检验交互作用显著性：交互作用要靠简单效应的差异来体现。交互作用显著代表两个简单效应有显著差异，而简单效应本身也是差值，差值是否为零另需检验。交互作用显著只能得出至少有一个简单效应不为零。一个简单效应显著（不为零），另一个简单效应不显著，不能得出他们的差值显著。

#### **对象内方差分析（Within-Subjects ANOVA）**
不同受试对象本身的变异可能远大于不同实验条件引起的变异，在同一个受试对象上重复测量的因子（repeated-measures factor）叫做对象内因子（within-subjects factor），可以避免无关变异干扰，增加实验有效性（Power）。
**方差分析汇总表（ANOVA Summary Table）之对象内单因子**
 - 变异来源有这几项：对象内因子，其他因子，Error。各项变异之和等于 Total 项变异。
 - Error 项就是所有因子交互项，自由度等于交互因子自由度乘积。
 - Total 项自由度等于所有因子水平数乘积减 1，等于以上各项自由度之和。

**方差分析汇总表（ANOVA Summary Table）之对象内与对象间多因子**
- 有两个 Error 项。

**滞后效应（Carryover Effects）**
受试对象的前一次测试会影响后续测试结果称为滞后效应（carryover effects），通过把测试顺序随机化可以平衡滞后效应。非对称滞后效应（Asymmetric carryover effects）是指随机化测试顺序无法解决的滞后效应，这种情况需要使用对象间因子设计。
